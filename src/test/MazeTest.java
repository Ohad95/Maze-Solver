...
public class MazeTest {
   ...
   private UnionFindTest uf;
   private DisplayImageTest image;
  ...
  /**
   ...
   public MazeTest (String fileName) {
       this.image = new DisplayImageTest(fileName);
       this.uf = new UnionFindTest(this.image.height() * this.image.width());
       int counter = 0;
       // First, we will determine whether the starting point is the pixels 0,0
       if (image.isRed(0, 0)) {
           this.startX = 0;
           this.startY = 0;
           counter++;
           this.image.set(0, 0, new Color(0xFF, 0xFF, 0xFF));
       }
       for (int i = 0; i <= this.image.height() - 1; i++) {
           for (int j = 0; j <= this.image.width() - 1; j++){
               int current = this.pixelToId(j, i);
        	   
               // In each iteration, we examine the two main endpoints
        	   //   1. When we are in the last column
        	   //   2. When we are in the last row
               if(i <= this.image.height()-2){
            	   //We will check whether the next pixel (to our right or below) is the red dot (start or end point)
                   ...
                   // Primary test - Does the neighboring pixel belong to the orbit 
                   // (in the color of the current pixel). If so, we will make a "union"
                       if (this.image.isOn(j,i) == this.image.isOn(j,i+1)){
                           int down = pixelToId(j, i+1);
                           uf.union(current, down);
                       }
                   }
                   if (j <= this.image.width()-2){
                	 //We will check whether the next pixel (to our right or below) is the red dot (start or end point)
                       if (image.isRed(j+1, i)) {
                           this.image.set(j+1, i, new Color(0xFF, 0xFF, 0xFF));
                           if(counter == 0) {
                               this.startX = j+1;
                               this.startY = i;
                               counter++;
                       ...
                       // Primary test - Does the neighboring pixel belong to the orbit 
                       // (in the color of the current pixel). If so, we will make a "union"
                       if (this.image.isOn(j,i) == this.image.isOn(j+1,i)){
                           int right = pixelToId(j+1, i);
   ...
   public void connect (int x1, int y1, int x2, int y2) {
	   int current1 = pixelToId(x1,y1);
	   int current2 = pixelToId(x2,y2);
	   uf.union(current1, current2);
   }
   ...
   public boolean areConnected (int x1, int y1, int x2, int y2) {
	   int current1 = pixelToId(x1,y1);
	   int current2 = pixelToId(x2,y2);
	   if (uf.find(current1) == uf.find(current2)) return true;
	   else	return false;
   }
   ...
   public boolean mazeHasSolution(){
		if(uf.find(pixelToId(this.startX,this.startY)) == uf.find(pixelToId(this.endX,this.endY)))
				return true;
		else return false;
   }
   /**
    ...
    */
   public DisplayImageTest getComponentImage() {

      DisplayImageTest compImg = new DisplayImageTest (image.width(), image.height());
